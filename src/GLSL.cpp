...
GLSL::GLSL()
: _ProgShaderID(0), _VertShaderID(0), _FragShaderID(0)
{   
    _NumAttrib = 0;
}
...
{
    glBindAttribLocation(_ProgShaderID, _NumAttrib, AttribName.c_str());
    _NumAttrib++;
}

void GLSL::Bind()
{
    glUseProgram(_ProgShaderID);
...
{
    GLint UniformLocation = glGetUniformLocation(_ProgShaderID, Name.c_str());
