...
void Text::_RemakeTexture()
{
    ...
    GameObjAssoc.Box.Redimension(Vec2(FontSurface->w, FontSurface->h));
    _Texture = SDL_CreateTextureFromSurface(Game::GetInstance().GetRenderer(), FontSurface);
...
void Text::Render()
{
    if(_Texture != nullptr)
    {
        ...
        SDL_Rect Destiny = {(int)(GameObjAssoc.Box.x - Game::GetInstance().GetState().Cam.Position.x),
            (int)(GameObjAssoc.Box.y - Game::GetInstance().GetState().Cam.Position.y),
            (int)GameObjAssoc.Box.w, (int)GameObjAssoc.Box.h};
        if(SDL_RenderCopyEx(Game::GetInstance().GetRenderer(), _Texture, &Clip, &Destiny, 
            Vec2::RadToDeg(GameObjAssoc.Angle),nullptr, (SDL_RendererFlip) Flip::N))
        {
